...
class Config:
    ...
    fantasycalc_base_url: str = field(
        default_factory=lambda: os.getenv("FANTASYCALC_BASE_URL", "https://api.fantasycalc.com")
    )

    dp_base_url: str = field(
        default_factory=lambda: os.getenv(
            "DP_BASE_URL", "https://raw.githubusercontent.com/dynastyprocess/data/master/files"
        )
    )
    dp_repo_url: str = field(
        default_factory=lambda: os.getenv("DP_REPO_URL", "https://github.com/dynastyprocess/data.git")
    )
    dp_crosswalk_url: str = field(
        default_factory=lambda: os.getenv(
            "DP_CROSSWALK_URL", "https://raw.githubusercontent.com/dynastyprocess/data/master/files/db_playerids.csv"
        )
    )

    nflverse_stats_url: str = field(
        default_factory=lambda: os.getenv(
            "NFLVERSE_STATS_URL",
    ...
    user_agent: str = field(
        default_factory=lambda: os.getenv("ETL_USER_AGENT", "dynasty-portfolio-etl/2.0 (+analytics-portfolio-project)")
    )
    ...
    backfill_previous_seasons: bool = field(
        default_factory=lambda: os.getenv("BACKFILL_PREVIOUS_SEASONS", "true").lower() == "true"
    )
